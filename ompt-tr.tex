...
\date{Revised September 16, 2015}
...
\begin{description}
  ...
  \item \verb|ompt_state_wait_barrier| (mandatory)
  ...
  \item \verb|ompt_state_wait_barrier_implicit| (optional)
...
\sloppy
  ...
  A  compliant runtime implementation may report \verb|ompt_state_wait_barrier| for implicit barriers.
    ...
    \item \verb|ompt_state_wait_barrier_explicit| (optional)
...
\item \verb|ompt_state_wait_target| (mandatory)
...
\item \verb|ompt_state_wait_taskwait| (mandatory)
...
\item \verb|ompt_state_wait_taskgroup| (mandatory)
...
\item \verb|ompt_state_wait_lock| (\verb|ompt_state_wait_nest_lock|) (mandatory)
...
\item \verb|ompt_state_wait_critical| (mandatory)
...
\item \verb|ompt_state_wait_atomic| (mandatory)
...
\item \verb|ompt_state_wait_ordered| (mandatory)
...
This callback has type signature \verb|ompt_target_task_end_callback_t|. The \verb|dev_task_id| passed to the callback is identical to  
the one passed to the matching \verb|ompt_event_target_task_begin| callback.
...
\end{quote}
Both 
\verb|ompt_get_task_info|  and  \verb|ompt_get_task_frame| identify a target task by a depth. Depth 0 refers to the current task. 
...
typedef void (*ompt_new_target_task_callback_t) (
  ...
  ompt_dev_activity_id_t dev_task_id,/* ID of target task on device */
  int device_id,                    /* ID of the device             */
  void *target_task_code            /* ptr to target code           */
);

typedef void (*ompt_target_task_end_callback_t) ( 	   
  ompt_dev_activity_id_t  dev_task_id /* ID of target task          */
);
