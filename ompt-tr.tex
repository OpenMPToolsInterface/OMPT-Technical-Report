...
\section{Target Device Records (optional)}
\label{sec:target-device-records}
The OMPT interface provides a portable method for data collection on target devices. 
The concept is based on asynchronous buffers that are managed by an OpenMP runtime implementation. 
...
If \verb|*buffer| is set to \verb|NULL|, the OpenMP runtime may drop all target device events.
%
...
When the buffer callbacks are registered all record types are enabled by default.

(Option to convert target device time stamps to host time stamps:
Pass a function pointer to a \verb|ompt_get_host_time| routine to the OpenMP runtime.)

\subsection{Retrieve Record Data}
\label{sec:target-record-data}

To retrieve records the following routine can be used to iterate over the buffer:
\begin{quote}
\begin{verbatim}
typedef ompt_record_t* ompt_get_next_record_t (
...
An OpenMP implementation might define more routines to get additional implementation-defined information.
We also need to specify OMPT record types. An OpenMP runtime implementation might define more record types.

\subsection{Enabling and Disabling Data Recording}
\label{sec:enabling-disabling-data-recording}
To enable and disable the recording of data:

\begin{quote}
\begin{verbatim}
typedef void ompt_record_enable_t (
  ompt_target_device_id_t device_id, /* target device id */
  ompt_record_type_t type            /* record type */
);
\end{verbatim}
\end{quote}
...
\begin{quote}
\begin{verbatim}
typedef void ompt_record_disable_t (
  ompt_target_device_id_t device_id, /* target device id */
  ompt_record_type_t type            /* record type */
);
\end{verbatim}
...
When no type is given (\verb|NULL| or defined \verb|OMPT_RECORD_NO_TYPE|?), all record types are enabled or disabled at once.
