...
\begin{quote}
\begin{verbatim}
OMPT_API ompt_target_record_type_t ompt_trace_record_get_type(
  ompt_target_buffer_t *buffer, 
  ompt_target_buffer_cursor_t current
);
\end{verbatim}
...
\verb|ompt_trace_record_get_type| returns either a known OMPT record type, \verb|ompt_target_record_notype| if the cursor is out of bounds, or  \verb|ompt_target_record_native| if the record represents a device native record type that does not represent an OMPT event record. To decode native records, one must use device-specific functions looked up using the lookup function returned by the \verb|ompt_get_device_info| inquiry function. 

To inspect the information in a trace record, a tool invokes the following function:
\begin{quote}
\begin{verbatim}
