...
\item \verb|ompt_event_task_dependence|

  The OpenMP runtime invokes this callback for every dependence that blocks execution of a task (\verb|succ_task_id|) because a predecessor task (\verb|pred_task_id|) has not yet completed  execution and  generated inputs required by \verb|succ_task_id|.
  This callback will always occur after creating a successor task (i.e. \verb|ompt_task_begin|) and before the successor task begins execution (i.e. its first \verb|ompt_task_switch|).
  This callback may occur even if the predecessor task has been descheduled (signaled by an \verb|ompt_task_switch| event) or if it has finished (signaled by an \verb|ompt_task_end| event).
  Note that this callback will not report information regarding dependences that have already been satisfied by completed tasks.
  This callback has type signature \verb|ompt_new_task_dependence_callback_t|.
...
typedef enum ompt_task_dependence_type_e {
   ompt_dependence_in    = 1,
   ompt_dependence_out   = 2,
   ompt_dependence_inout = 3
...
/* task dependences */
typedef void (*ompt_new_task_dependence_callback_t) (
                                     /* new dependence tracking     */
  ompt_task_id_t pred_task_id,       /* ID of predecessor task      */
  ompt_task_id_t succ_task_id,       /* ID of successor task        */
  ompt_task_dependence_type_t type,  /* Type of dependence          */
