...
\sloppy
  This state is a placeholder exclusively reserved for use by the OMPT runtime call \verb|ompt_enumerate_states| (see Section~\ref{ompt_enumerate_states}), which is used to enumerate all available runtime states. A thread will never be reported in this state.
...
the value \verb|omp_state_first| should be supplied for \verb|current_state| in the call to \verb|ompt_enumerate_states| that begins the enumeration.
...
To begin enumerating the mutex implementations that a runtime  employs, one should supply 
 \verb|ompt_mutex_kind_first| as the value for \verb|current_kind| in the first call to \verb|ompt_enumerate_mutex_kinds|.
...
\begin{quote}
\begin{verbatim}
uint32_t kind = ompt_mutex_kind_first;
...
If there is no enclosing parallel region at the specified ancestry level, the values of \verb|*parallel_id| and \verb|*team_size| are undefined.
{\em This function is async signal safe.}
...
\end{verbatim}
\end{quote}
This function returns the value 0 if the thread invoking it is outside a target region.  {\em This function is async signal safe.}
...
typedef uint64_t ompt_target_activity_id_t; /* uniquely identifies an activity on a device   */
...
#define ompt_mutex_kind_first         0
#define ompt_mutex_hint_unknown       (~0)
\end{verbatim}
\clearpage
...
typedef void (*ompt_target_data_callback_t) (
  ...
  ompt_target_optype_t optype,      /* type of operation            */
