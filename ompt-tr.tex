...
Function \verb|ompt_get_device_info| is used to find out information about an attached device.
\begin{quote}
\begin{verbatim}
...
Function \verb|ompt_get_target_device_time| returns the current time stamp on  device \verb|device_id|:
\begin{quote}
\begin{verbatim}
...
Host and target devices are typically distinct and run independently. If the
...
to align time stamps from the target device with time
...
\end{itemize}
 ...
 Section~\ref{sec:trace-event}  describes how to enable or disable tracing on a target device for specific event types. Section~\ref{sec:trace-startstop} describes how to start and stop event tracing on a device. All functions in the target device tracing control API in this section are marked with \verb|OMPT_API|. These functions should not be global symbols in an OpenMP runtime system implementation to avoid tempting tool developers to call them directly. Section~\ref{sec:init} describes how a tool will obtain pointers to these functions.


\subsection{Enabling and Disabling Tracing for Specific Record Types}
\label{sec:trace-event}
Function \verb|ompt_record_set_t| enables a tool to enable or disable the recording of one or more types of trace record types:
...
\end{verbatim}
\end{quote}
The argument \verb|device_id| indicates the target device whose performance monitoring may be altered by invoking this function. The boolean \verb|enable| indicates whether tracing of records of type \verb|rtype| may be enabled or disabled by this invocation.
...
If the function \verb|ompt_record_set_t| is called for a device for which tracing is currently active, its behavior is undefined.

\subsection{Starting and Stopping Tracing}
\label{sec:trace-startstop}

To start tracing a tool needs to register a \emph{buffer request} callback that will supply a target with a buffer to deposit trace events and a \emph{buffer complete} callback that will be invoked by the OpenMP runtime system to empty a trace buffer containing trace records. The \emph{buffer request} callback has the following type signature:
\begin{quote}
\begin{verbatim}
...
\end{verbatim}
\end{quote}
As necessary, the OpenMP runtime will asynchronously invoke \verb|ompt_target_buffer_request_callback_t| to request a buffer to store trace records for target device \verb|device_id|. \marginpar{What thread will execute these callbacks? Should it be specified? When are these callbacks permitted and prohibited?}
% The argument \verb|buffer| to the buffer request callback is the address of a pointer to a buffer. 
A tool should set \verb|*buffer| to point to a buffer where target device events may be recorded and \verb|*bytes| to the length of that buffer.
...
A buffer request callback  may set \verb|*bytes| to 0 if it does not want to provide a buffer for any reason. If a callback sets \verb|*bytes| to 0, further event tracing for the device will be disabled until the next invocation of \verb|ompt_target_tracing_start|. This will cause the OpenMP runtime to drop future trace records for a target device until tracing is restarted.
...
\end{verbatim}
\end{quote}
The OpenMP runtime triggers \verb|ompt_target_buffer_complete_callback_t| when no further records will be recorded in a trace buffer and all records written to the buffer are valid. 
The argument \verb|device_id| indicates the device whose trace events the buffer contains. The argument \verb|buffer| is the address of a buffer previously allocated by the \emph{buffer request} callback. The argument \verb|bytes| indicates the full size of the buffer. The arguments \verb|begin| and \verb|end|, respectively, are opaque cursors that indicates the position at the beginning of the first trace record in the buffer and the position after the last trace record in the buffer. If \verb|begin| is equal to \verb|end|, the buffer is empty. \marginpar{What thread will execute these callbacks? Should it be specified? When are these callbacks permitted and prohibited?}

% It is implementation-defined when an OpenMP runtime triggers this callback. 
Under normal operating conditions, every trace buffer provided to a device by a \emph{buffer request} callback will receive a \emph{buffer complete} callback before the OpenMP runtime shuts down. If an exceptional condition terminates  execution of an OpenMP program, the OpenMP runtime may choose to not provide a \emph{buffer complete} callback for buffers provided to any device.

To start and stop tracing for a specific target device, a tool calls the functions \verb|ompt_target_trace_start| and \verb|ompt_target_trace_stop| with the following type signatures:
\begin{quote}
\begin{verbatim}
OMPT_API int ompt_target_trace_start (
  int device_id, 
...
\begin{quote}
\begin{verbatim}
OMPT_API int ompt_target_trace_stop(
  int device_id 
);
\end{verbatim}
\end{quote}
...
\label{sec:target-record-data}

There are several routines that need to be used together process target trace records deposited in a buffer by a device. First, function \verb|ompt_trace_record_cursor_advance|, with the type signature shown below:

\begin{quote}
\begin{verbatim}
OMPT_API int ompt_trace_record_cursor_advance(
  ompt_trace_buffer_t *buffer, 
  ompt_target_buffer_cursor_t current, 
  ompt_target_buffer_cursor_t *next
); 
\end{verbatim}
\end{quote}
advances from one trace record in a buffer to the next. It returns 0 if the advance is successful and 1 if \verb|current| indicates a position before the first record of the buffer or a position at or beyond the last record in the buffer.

The function \verb|ompt_trace_record_kind| enables a tool to inspect the type of a record to determine whether it is both a known type and worth processing further. 
\begin{quote}
\begin{verbatim}
OMPT_API ompt_target_record_type_t ompt_trace_record_type(
  ompt_target_buffer_t *buffer, 
  ompt_target_buffer_cursor_t current
);
\end{verbatim}
...
\verb|ompt_trace_record_type| returns either a known OMPT record type, \verb|ompt_target_record_notype| if the cursor is out of bounds, or  \verb|ompt_target_record_native| if the record represents a device native record type that does not represent an OMPT event record. To decode native records, one must use device-specific functions looked up using the lookup function returned by the \verb|ompt_get_device_info| inquiry function. 

\begin{quote}
\begin{verbatim}
OMPT_API ompt_trace_record_t *ompt_trace_record_get(
  ompt_target_buffer_t *buffer, 
  ompt_cursor_t current
);
\end{verbatim}
\end{quote}
The pointer returned by \verb|ompt_trace_record_get| may point into the trace buffer, or it may point into thread local storage where the record was constructed. Another call to \verb|ompt_trace_record_get| may overwrite the contents of the fields in the previous record returned.
...
typedef struct {
   ...
   ompt_parallel_id parallel_id,
   ompt_task_id_t parent,
   ompt_task_id_t current,
   ompt_task_frame_t parent_frame,
   uint32_t team_size
} ompt_trace_record;
\end{verbatim}
\end{quote}
An OMPT event will set the fields in this record that would be provided in a callback for the event, as well as the timestamp, thread, and current task.

\begin{comment}

(Input record to continue the buffer iteration after given record or NULL to start at buffer begin)

\begin{quote}
\begin{verbatim}
typedef void ompt_get_next_record_t (
  ompt_record_t* buffer,   /* pointer to record buffer */
  ompt_record_type_t type, /* record type */
  ompt_record_t **record   /* in/out record, NULL to start at buffer begin */
);
\end{verbatim}
\end{quote}

Depending on the record type, specific record data are available that can be obtained via getter-routines:

\begin{description}
\sloppy

\item \verb|uint64_t           ompt_record_get_time_begin_t( ompt_record_t *record )|

\item \verb|uint64_t           ompt_record_get_time_end_t( ompt_record_t *record )|

\item \verb|ompt_thread_id_t   ompt_record_get_thread_id_t( ompt_record_t *record )|

\item \verb|ompt_thread_type_t ompt_record_get_thread_type_t( ompt_record_t *record )|

\item \verb|ompt_parallel_id_t ompt_record_get_parallel_id_t( ompt_record_t *record )|

\item \verb|ompt_task_id_t     ompt_record_get_task_id_t( ompt_record_t *record )|

\item \verb|ompt_task_id_t     ompt_record_get_parent_task_id_t( ompt_record_t *record )|

\item \verb|ompt_task_id_t     ompt_record_get_suspended_task_id_t( ompt_record_t *record )|

\item \verb|ompt_task_id_t     ompt_record_get_resumed_task_id_t( ompt_record_t *record )|

\item \verb|void*              ompt_record_get_symbol_t( ompt_record_t *record )|

\item \verb|ompt_frame_t*      ompt_record_get_parent_task_frame_t( ompt_record_t *record )|

\item \verb|uint32_t           ompt_record_get_requested_team_size_t( ompt_record_t *record )|
\end{description}
\end{comment}
...
\clearpage  
\subsection{Target Record Types and Buffer Management}
...
typedef struct
{
  ...
  union
  {
    ompt_target_thread_t;                      /* for thread          */
    ompt_target_thread_type_t;                 /* for thread type     */
    ompt_target_wait_t;                        /* for wait            */
...
{
  ompt_thread_id_t thread_id       /* ID of thread                  */
} ompt_target_thread_t;

typedef struct
{
  ompt_thread_type_t thread_type,  /* type of thread               */
  ompt_thread_id_t thread_id       /* ID of thread                 */
} ompt_target_thread_type_t;

typedef struct {
  ompt_wait_id_t wait_id           /* wait ID                      */
} ompt_target_wait_callback_t;
