...
\item \verb|ompt_state_wait_target_map| 
...
A runtime implementation may report a thread waiting for any type of mutual exclusion using either state \verb|ompt_state_wait_mutex| or \verb|ompt_state_wait_lock|. 
...
\item \verb|ompt_state_wait_mutex|
...
\sloppy
The OpenMP runtime invokes this callback
after an explicit or target task completes. 
The callback executes in the context of the task that will next execute on
the thread where the task completed.
...
The OpenMP runtime invokes this callback after a task encounters any target construct. The callback executes in the 
context of the task that will initiate the work of the target construct.
This callback has type signature \verb|ompt_target_begin_callback_t|.

\item \verb|ompt_event_target_end|   
 
\sloppy
The OpenMP runtime invokes this callback after the work of a target construct completes on both the host and the device.
%The callback executes in the context of the task that encountered the target task construct.
This callback has type signature \verb|ompt_target_end_callback_t|.
...
\item \verb|ompt_event_initial_task|
   ...
   The OpenMP runtime invokes this callback with the \verb|endpoint=|\verb|ompt_scope_end| after 
...
\item \verb|ompt_event_task_switch|
 ...
 callback executes in the context of the resumed task.  
  ...
  The callback argument \verb|second_task_id| indicates the task being resumed. 
  The callback argument \verb|first_task_id| indicates the task being suspended or \verb|task_id_none| if
  the preceding task executed by the thread completed.
...
\item \verb|ompt_event_target_data_begin|
  ...
  This callback has type signature \verb|ompt_target_data_begin_callback_t|. Arguments to the callback include the encountering task, the  
...
\item \verb|ompt_event_target_data_map|
  ...
  The OpenMP runtime invokes this callback just before a variable is mapped to or unmapped from the device data environment by a target, target data, 
  target enter data or target exit data construct. The argument \verb|kind| indicates whether the callback is to map or unmap the data.
%  The argument \verb|mapid| is used to correlate the initiation of a map operation on a host with activity on a device.
  The callback executes in the context of the encountering task.
  
  \item \verb|ompt_event_target_data_update|

  This callback has type signature \verb|ompt_target_data_update_callback_t|. 
  The OpenMP runtime invokes this callback just before a variable is updated to/from the device data environment by a target update construct. 
  The argument \verb|kind| indicates whether the callback is to move data to or from the device.
  The callback executes in the context of the encountering task.
...
\item \verb|ompt_event_init_lock|
   ...
   \verb|ompt_enumerate_sync_kinds|. 
...
\noindent
  ...
  OpenMP runtime is initialized until it is shut down. 
  A parallel region identifer can be retrieved
  ...
  Each OpenMP task has an associated identifer of type
...
\subsection{Wait Identifier}
  ...
  Each thread instance maintains a {\em wait identifier} of type \verb|ompt_wait_id_t|. When a task executing on a thread is waiting for something, the thread's wait identifer indicates what the thread is awaiting. A wait identifier may represent a critical section {\em name}, a lock,  a program variable accessed in an atomic region, or a synchronization object internal to an OpenMP runtime implementation. 
\begin{comment}
\begin{quote}
\begin{verbatim}
...
\end{quote}
\end{comment}
 A thread's wait identifier can be retrieved on
...
\subsection{Structure to Support Classification of Stack Frames}
  ...
  When executing an OpenMP program, at times, procedure frames from the OpenMP runtime appear on the call stack between procedure frames of user code.
  ...
  region or task does not create a new task. A degenerate task may arise is when a parallel construct is encountered
...
\end{quote}
 ...
 \label{ompt_enumerate_sync_kinds}
 
The OpenMP standard recognizes that a runtime system may implement locks, nest locks, critical sections, and atomic regions in several different ways. For that reason, a user program can provide the runtime system with hints that help the runtime system to select appropriate implementations.
...
\begin{quote}
\begin{verbatim}
OMPT_API int ompt_enumerate_sync_kinds(
...
\noindent
\sloppy
To begin enumerating the synchronization implementations that a runtime  employs,
 \verb|ompt_sync_kind_first| should be supplied as the initial value for \verb|current_kind| in the call to \verb|ompt_enumerate_sync_kinds| to  begin the enumeration.
...
The argument \verb|next_kind_name| is a pointer to a location that will be filled in with a pointer to the name of the synchronization implementation associated with \verb|next_kind|. 
Each subsequent invocation of \verb|ompt_enumerate_sync_kinds| should pass the code returned in \verb|next_kind| by the prior call.
Whenever one or more synchronization implementations are left in the enumeration, \verb|ompt_enumerate_sync_kinds| will return 1.
When the last synchronization implementation type in the enumeration is passed to \verb|ompt_enumerate_sync_kinds| as \verb|current_kind|, 
the function will return 0 indicating that the enumeration is complete.
An example of how to enumerate the types of synchronization implementations supported by an OpenMP runtime is shown below:

\begin{quote}
\begin{verbatim}
uint32_t kind = ompt_sync_kind_first;
const char *kind_name;
while (ompt_enumerate_sync_kinds(kind, &kind, &kind_name)) {
...
Function \verb|ompt_get_parallel_id| returns the identifier associated with an enclosing parallel region. 
...
ancestor level, one can access information about all enclosing parallel regions. 
The meaning of different values for the \verb|ancestor_level| argument to \verb|ompt_get_parallel_id| is given in Table~\ref{tab:ancestor}.
If there is no enclosing parallel region at the specified ancestry level,  \verb|ompt_get_parallel_id| will return 0. {\em This function is async signal safe.}
...
\noindent
 This function returns the value -1 if there is no enclosing parallel region at the specified ancestry level. {\em This function is async signal safe.}
...
A task may be of type initial, implicit, explicit,  target, or degenerate. If the task at the specified  level is degenerate, the value returned in \verb|task_id| will be 0. A degenerate task will be associated with the enclosing parallel region. If the task is outside any parallel region, the value returned in \verb|par_id| will be 0.
...
\end{quote}
\sloppy
\verb|ompt_target_buffer_get_record_type| returns either \verb|ompt_record_ompt| if the record represents a OMPT event,  \verb|ompt_record_correlation| if the purpose of the record is to associate the identier for a device activity with a host target task id, \verb|ompt_record_native| if the record represents a device native record type that does not represent an OMPT event record, or \verb|ompt_record_invalid| if the cursor is out of bounds. 
...
typedef enum ompt_state_e {
 ...
 /* task wait states (80..95) */
  ...
  ompt_state_wait_mutex             = 0x60, /* waiting for any mutex kind */      	    
  ompt_state_wait_lock              = 0x61, /* waiting for lock           */
...
typedef enum ompt_event_e {  
  ...
  ompt_event_task_begin               = 5,  
  ompt_event_task_end                 = 6,   
  ompt_event_target_begin             = 7,  
  ompt_event_target_end               = 8,  
  ompt_event_control                  = 9,   
  ompt_event_runtime_shutdown         = 10,  

  /*--- Optional Events for Blame Shifting ---*/  	
  ompt_event_idle	                    = 11,   
  ompt_event_sync_region_wait         = 12, 
  ompt_event_mutex_release            = 13, 

  /*--- Optional Events for Instrumentation-based Tools --- */      	    
  ompt_event_implicit_task            = 14,  
  ompt_event_initial_task             = 15,  				    
  ompt_event_task_switch              = 16,   
  ompt_event_task_dependences         = 17,  
  ompt_event_task_dependence_pair     = 18   				    				   	
  ompt_event_worksharing              = 19,  		              	 			    
  ompt_event_master                   = 20,  
  ompt_event_target_data_map          = 21, 	          	 			   
  ompt_event_sync_region              = 22,  
  ompt_event_init_lock                = 23,  
  ompt_event_destroy_lock             = 24, 								    
  ompt_event_mutex_acquire            = 25,  
  ompt_event_mutex_acquired           = 26,  
  ompt_event_nested_lock              = 27,   			            	 			         
  ompt_event_flush                    = 28  
} ompt_event_t;
\end{verbatim}
...
#define ompt_lock_hint_unknown (~0)
\end{verbatim}
\clearpage
...
typedef void (*ompt_scoped_mutex_acquire_callback_t) ( 
  ompt_mutex_kind_t kind,
  uint32_t hint,
  uint32_t impl,  
  ompt_wait_id_t wait_id,
...
typedef void (*ompt_scoped_mutex_acquired_callback_t) ( 
  ompt_mutex_kind_t kind,   
  ompt_wait_id_t wait_id,
  const void *codeptr_ra            /* return address of api call   */          
);
...
typedef void (*ompt_lock_init_callback_t) (
  ...
  ompt_lock_impl_t kind,            /* implementation               */
  const void *codeptr_ra            /* return address of api call   */
);
...
/* enumerate kinds of mutex implementations */
OMPT_API int ompt_enumerate_sync_kinds(  
  uint32_t current_kind, 
  uint32_t *next_kind,   
  const char **next_kind_name    
...
typedef enum ompt_record_type_e {
  ompt_record_ompt = 1,
  ompt_record_correlation = 2,
  ompt_record_native = 3,
...
\begin{itemize}
\item are the target callback notifications satisfactory?
\item association of device activities with mappings and the source code? do we need a new correlation event? a map callback may occur outside a target task. Do we want to have the tool provide the host correlation ID to the device rather than have the runtime do it?
\item design of target update callback?
\item is it ok to do away with target data and just have the mapping callback?
\item taskloop?
\item taskyield?
\item simd
\item distribute
\item teams?
\item should the callbacks for implicit tasks be added to the mandatory set?
