...
\end{verbatim}
\end{quote}
As necessary, the OpenMP runtime will asynchronously invoke \verb|ompt_target_buffer_request_callback_t| to request a buffer to store trace records for target device \verb|device_id|.
...
The argument \verb|device_id| indicates the device whose trace events the buffer contains. The argument \verb|buffer| is the address of a buffer previously allocated by the \emph{buffer request} callback. The argument \verb|bytes| indicates the full size of the buffer. The arguments \verb|begin| and \verb|end|, respectively, are opaque cursors that indicates the position at the beginning of the first trace record in the buffer and the position after the last trace record in the buffer. If \verb|begin| is equal to \verb|end|, the buffer is empty. 
% \marginpar{What thread will execute these callbacks? Should it be specified? When are these callbacks permitted and prohibited?}
...
There are several routines that need to be used together process target trace records deposited in a buffer by a device. Function \verb|ompt_trace_record_cursor_advance|, with the type signature shown below,

\begin{quote}
\begin{verbatim}
OMPT_API int ompt_trace_record_cursor_advance(
  ompt_trace_buffer_t *buffer, 
  ompt_trace_buffer_cursor_t current, 
  ompt_trace_buffer_cursor_t *next
); 
\end{verbatim}
\end{quote}
advances from one trace record in the specified buffer to the next. It returns 0 if the advance is successful and 1 if \verb|current| indicates a position before the first record of the buffer or a position at or beyond the last record in the buffer.

The function \verb|ompt_trace_record_get_type| enables a tool to inspect the type of a record to determine whether it is both a known type and worth processing further. 
\begin{quote}
\begin{verbatim}
OMPT_API ompt_record_type_t ompt_trace_record_get_type(
  ompt_trace_buffer_t *buffer, 
  ompt_trace_buffer_cursor_t current
);
\end{verbatim}
...
\verb|ompt_trace_record_get_type| returns either \verb|ompt_trace_record_ompt| if the record represents a OMPT event, \verb|ompt_trace_record_notype| if the cursor is out of bounds, or  \verb|ompt_trace_record_native| if the record represents a device native record type that does not represent an OMPT event record, Appendix~\ref{appendix:ompt-target-device-events} defines the corresponing enumeration type for \verb|ompt_trace_record_type_t|.
...
To inspect the information in for an OMPT trace record event, a tool invokes the following function:
\begin{quote}
\begin{verbatim}
OMPT_API ompt_record_t *ompt_trace_record_get(
  ompt_trace_buffer_t *buffer, 
  ompt_cursor_t current
);
\end{verbatim}
\end{quote}
This function returns a pointer that may point into the trace buffer, or it may point into thread local storage where the information extracted from a trace record was assembled. The information available for a trace event depends upon its type. For this purpose, Appendix~\ref{appendix:ompt-target-device-events} defines a union type that will be used to return information for different OMPT trace event record types. A subsequent call to \verb|ompt_trace_record_get| may overwrite the contents of the fields in the record returned by a prior invocation.

\textcolor{red}{TODO: Define a function for native event processing: OMPT\_API void *ompt\_trace\_record\_get\_native ?}
...
\clearpage  
\subsection{Trace Record Types and Buffer Management}
\label{appendix:ompt-target-device-events}

\begin{verbatim}
/* OMPT record type */
typedef enum {
  ompt_trace_record_ompt,
  ompt_trace_record_native,
  ompt_trace_record_notype
} ompt_trace_record_type_t; 
\end{verbatim}
