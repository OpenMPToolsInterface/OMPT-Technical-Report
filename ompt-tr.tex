...
\begin{quote}
\begin{verbatim}
typedef ompt_target_record_t* ompt_target_get_next_record_t (
  void* buffer
);
\end{verbatim}
\end{quote}
...
OR: \\
CUPTI allows an input record to continue the buffer iteration after given record or NULL to start at buffer begin.

\begin{quote}
\begin{verbatim}
typedef void ompt_target_get_next_record_t (
  void* buffer,
  ompt_target_record_t **record
);
\end{verbatim}
\end{quote}

The first field of a record is the record type.

\begin{quote}
\begin{verbatim}
typedef struct {
  ompt_record_type_t type;
} ompt_record_t;
\end{verbatim}
\end{quote}

Depending on the record type, specific record data are available that are obtained via getter-routines:
...
Do we have a vendor-independent way to request for additional getter-routines?
...
typedef void ompt_record_enable_t (
  ...
  ompt_record_type_t type    /* there is no list of types */
);
\end{verbatim}
\end{quote}
...
typedef struct
{
  ...
  union
  {
    ...
    ompt_target_thread_type_t;                 /* for thread          */
