...
\item \verb|ompt_event_target_data_map_begin|
  ...
  This callback has type signature \verb|ompt_data_map_begin_callback_t|.  
...
\item \verb|ompt_event_target_data_map_end|
  ...
  This callback has type signature \verb|ompt_data_map_end_callback_t|. The \verb|map_id| passed to the callback is the same as the one provided to the task that initiated the mapping.
...
\item \verb|ompt_event_target_data_map_done_sync|
  ...
  The OpenMP runtime invokes this callback after a synchronous variable map operation finishes execution on a device. This callback has type signature \verb|ompt_data_map_end_callback_t|.  The \verb|map_id| passed to the callback is the same as the one provided to the task that initiated the mapping.
\end{comment}
...
The function \verb|ompt_translate_target_device_time| is used to translate a device time to a host time. The {\tt double} result for the host time has the same meaning as the {\tt double} returned from \verb|omp_get_wtime|.
\begin{quote}
\begin{verbatim}
OMPT_API 
double ompt_translate_target_device_time(
  int device_id, 
  ompt_target_device_time_t time
);
\end{verbatim}
\end{quote}
\noindent
...
Function \verb|ompt_trace_set_ompt| allows a tool to enable or disable the recording of one or more types of trace record types:

\begin{quote}
\begin{verbatim}
OMPT_API int ompt_trace_set_ompt(
...
Table~\ref{table:record_set} shows the possible return codes for \verb|ompt_trace_set_ompt|.


\subsection{Enabling and Disabling Tracing for Native Record Types}
\label{sec:trace-event-native}
Function \verb|ompt_trace_set_native| allows a tool to enable or disable the recording of native trace records for a device. This interface is designed for use by a tool with no 
...
\begin{quote}
\begin{verbatim}
OMPT_API int ompt_trace_set_native(
  int device_id,                     /* target device id                */
  ompt_bool enable,                  /* enable or disable               */
...
A buffer request callback  may set \verb|*bytes| to 0 if it does not want to provide a buffer for any reason. If a callback sets \verb|*bytes| to 0, further recording of events for the device will be disabled until the next invocation of \verb|ompt_trace_start|. This will cause the OpenMP runtime to drop future records for a device until recording is restarted.
...
To start, pause, or stop tracing for a specific device, a tool calls the functions \verb|ompt_trace_start|, \verb|ompt_trace_pause|, or \verb|ompt_trace_stop| with the following type signatures:
\begin{quote}
\begin{verbatim}
OMPT_API int ompt_trace_start(
  int device_id, 
...
\begin{quote}
\begin{verbatim}
OMPT_API int ompt_trace_stop(
  int device_id 
);
\end{verbatim}
\end{quote}
\noindent
\verb|ompt_trace_stop| asks the device to flush any buffers it has in its possession.
...
event records deposited in a buffer by a device. Function \verb|ompt_trace_cursor_advance|, with the type signature shown below,

\begin{quote}
\begin{verbatim}
OMPT_API int ompt_trace_cursor_advance(
  ompt_buffer_t *buffer, 
  ompt_trace_cursor_t current, 
  ompt_trace_cursor_t *next
); 
\end{verbatim}
\end{quote}
...
The function \verb|ompt_trace_get_type| enables a tool to inspect the type of a record to determine whether it is both a known type and worth processing further. 
\begin{quote}
\begin{verbatim}
OMPT_API ompt_record_type_t ompt_trace_get_type(
  ompt_buffer_t *buffer, 
  ompt_trace_cursor_t current
);
\end{verbatim}
...
\begin{quote}
\begin{verbatim}
OMPT_API ompt_record_ompt_t *ompt_trace_get_ompt(
  ompt_buffer_t *buffer, 
  ompt_trace_cursor_t current
);
\end{verbatim}
...
\begin{quote}
\begin{verbatim}
OMPT_API ompt_trace_correlation_t *ompt_trace_get_correlation(
  ompt_buffer_t *buffer, 
  ompt_trace_cursor_t current
);
\end{verbatim}
...
\begin{quote}
\begin{verbatim}
OMPT_API void *ompt_trace_get_native(
  ompt_buffer_t *buffer, 
  ompt_trace_cursor_t current
);
\end{verbatim}
...
typedef void ompt_buffer_t; /* buffer where device events my be recorded */

typedef void* ompt_trace_cursor_t; /* pointer to a buffer element */
...
typedef struct ompt_map_entry_s {
...
typedef void (*ompt_data_map_begin_callback_t) (
  ompt_task_id_t task_id,           /* ID of encountering task      */
  int device_id,                    /* ID of the device             */
  ompt_map_entry_t *items, /* the items to be mapped */
  uint32_t nitems, /* the number of items to be mapped */
  ompt_dev_activity_id_t  map_id    /* ID for map event             */
);

typedef void (*ompt_data_map_end_callback_t) (
  int device_id,                    /* ID of the device             */
  ompt_dev_activity_id_t  map_id    /* ID for map event             */
);
...
/* trace buffer management routines */
typedef void (*ompt_buffer_request_callback_t) (
  int device_id,                  /* target device                  */
  ompt_buffer_t** buffer,         /* pointer to host memory to store 
                                     target records                 */
...
typedef void (*ompt_buffer_complete_callback_t) (
  int device_id,             /* target device                       */
  ompt_buffer_t* buffer,     /* pointer to buffer with target 
                                  event records                     */
  size_t bytes,              /* number of valid bytes in the buffer */
  ompt_trace_cursor_t begin  /* first entry of the buffer           */
);
\end{verbatim}
%Tim: Do we really need ompt_buffer_{request,complete}_callback_t here? It is already defined above.
...
OMPT_API ompt_trace_type_t ompt_trace_get_type(
  ompt_buffer_t *buffer,
  ompt_trace_cursor_t current
);

OMPT_API ompt_trace_ompt_t *ompt_trace_get_ompt(
  ompt_buffer_t *buffer,
  ompt_trace_cursor_t current
);
...
OMPT_API ompt_trace_native_abstract_t ompt_trace_get_native_abstract(
  void *native_record
);
\end{verbatim}
...
typedef struct ompt_record_native_abstract_s {
  ...
  const char *type;
  ompt_target_device_time_t start_time;
  ompt_target_device_time_t end_time;
  ompt_dev_activity_t dev_task_id;
  uint64_t hwid;
...
typedef struct ompt_record_correlation_s {
  ...
  ompt_dev_activity_t dev_task_id;
} ompt_trace_correlation_t;

/* OMPT record */
typedef struct ompt_record_ompt_s {
  ompt_event_t type;                    /* event type                        */
  ompt_target_device_time_t time;   /* time, when the record occurred    */
  ompt_thread_id_t thread_id;           /* thread identifier for this record */
  ompt_dev_activity_id_t dev_task_id;     /* link back to its host context     */
  union
  {
